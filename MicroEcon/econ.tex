...
\begin{outline*}
\1 Fixed inputs are those whose quantity cannot be changed during the short-run, such as land, while variable are those able to varied, such as labor
...
\2 Total cost curve is the total cost (variable, since fixed is held constant) on the y-axis and the quantity produced on the x-axis, getting steeper as it progresses due to diminishing returns for added variable input (increasing marginal cost)
...
\end{outline*}
\subsection{Long Run Costs and Scale Economies}
\begin{outline*}
\1
\end{outline*}
