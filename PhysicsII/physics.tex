...
\2 As an example for some light beam, due to the fixed speed, if relative to one viewer, the light is only  moving on a single axis to the detector in that direction, whereas to another viewer, it is also moving in another axis along with the detector, due to the light traveling further, it would be measured as taking longer
\3 The measurement clocks must also be synchronized, or the time from the additional light distance would be added an extra time
\2 Thus, the distance traveled relative to the frame of reference changes the time measured, rather than the time remaining static
\2 It is thus derived from the respective distances, using a fixed velocity of light, that $\Delta t = \frac{\Delta t_0}{\sqrt{1 - (\frac{v}{c})^2}}$, where v is the relative speed of the horizontally-moving frame of reference
\3 For simplification, $\beta = \frac{v}{c}$, called the speed parameter, and $\gamma = \frac{1}{\sqrt{1 - \beta^2}}$, called the Lorentz factor, such that $\Delta  t = \gamma \Delta t_0$
\2 Measurements of the time interval between two events in a single location within an inertial reference frame, called the rest frame, is called the proper time interval, such that all other inertial reference frame measurements are greater
\3 The difference between the proper time interval and the measured time is the time dilation
\3 Generally, relativity time dilation must be taken note of in calculations when $v > 0.1c$
\3 Due to the relativity of simultaneity, it would appear to the proper observer that the clocks  were unsynchronized, accounting for the different measurements
\2 Time dilation can be seen in microscopic clocks, such as unstable muon particles, measured when stationary to have a lifetime before decay of 2.2 $\micro s$ as the proper time interval
\3 Due to being able to be moved at near light speeds, time dilation can cause drastically greater measured lifetimes than the proper time
\2 In October 1971, Hafele and Keating flew ultra-precise atomic clocks around the world in different directions on commercial airlines to test general relativity to within 10\% error, repeated later by University of Maryland by flying around Chesapeake Bay for 15 hours, getting precise of 1\%
\1 Length relativity/contraction is due to the necessity of measuring the distance between endpoints simultaneously, relative due to the relativity of simultaneity
\2 This can also be expressed as a direct result of temporal relativity
\2 Thus, it is in terms of the proper length, where the object being measured is moving, such that the two events occur in the same place with respect to the observer, rather than requiring synchronized clocks at each location
\3 The time measured by the moving source as a result is less than that of the stationary observer, due to time dilation, such that the length is decreased
\3 As a result, it is found that $L = \frac{L_0}{\gamma}$
\1 By temporal relativity, Galilean transformations for conversion to a different inertial frame of reference (for S' where $\vec{v} = \vec{v}_{S_x}$, $x' = x - vt, t' = t$) must be modified to work with the theory of relativity, such that they are Lorentz transformations ($x' = \gamma (x - vt), t' = \gamma (t - \frac{vx}{c^2}$)
\2 This assumes that the location is at the origin when t = 0, though it follows that $\Delta x'$ and $\Delta t'$ use the same general formulas
\2 By the time equation, events can be simultaneous in the S' frame, if $\Delta t = \gamma \frac{v}{\Delta x'}{c^2}$, appearing separate in S based on the distance separation in S', showing the relativity of simultaneity
\2 In addition, time dilation is shown by if $\Delta x = 0$ to get the proper time interval as t, $t \gamma = t'$ showing the time dilation equation from the transformation
\3 Similarly, length contraction is shown by if $\Delta t = 0$ to get a proper length measurement, $\Delta x' = \gamma x$, and since S' is
\2 These transformations are interpreted such that if some object is moving, it can be thought to be stationary the entire frame moving and vice versa at any velocity
\3 This makes it such that events are measured by the stationary point of view for the object, but from the other frame for those in another frame
\2 These formulas for transformation can be combined to find a velocity transformation, such that for some velocity in frame S, $\frac{dx}{dt}$, in S', $\frac{dx'}{dt'} = \frac{\frac{dx}{dt} - v}{1 - \frac{v}{c^2}\frac{dx}{dt}}$
\3 This can be used to find the classical form, such that as $c \to \infty$, $\frac{dx'}{dt'} = \frac{dx}{dt} - v$, used as the method to derive all classical transformations
\1 It is noted that causal relationships must have the traveling of information from the cause to the result, such that if the events happen at a rate above c, it isn't causal
\1 The Doppler effect for light does not depend on the medium, such that it only depends on the relative velocity of the source and observer, rather than with respect to the medium as for other waves
\2 Thus, for some frequency in the stationary frame with respect to the source, $f_0$, then $f =  f_0sqrt{\frac{1 - \frac{v}{c}}{1 + \frac{v}{c}}} = f_0\sqrt{\frac{1 - \beta}{1 + \beta}}$, where v is the velocity of observer with respect to the source, moving directly radial to the observer
\2 By power series, it can be expanded for speeds far less than the speed of light, such that $f = f_0(1 - \beta + \frac{1}{2}\beta^2)$, similar to that of the Doppler effect of other waves, except with a different third coefficient, showing the relativistic effect
\3 This is used by police radar emitting microwaves, which then reflect the beam at the wavelength by which it receives it back to the scanner, higher than the initial, after which the radar detects it at a further high wavelength due to the second Doppler effect
\3 Similarly, for astronomical bodies, due to the drastically lower velocity, $f = f_0(1 - \beta)$, or $\gamma = \gamma_0(1 + \beta)$, such that for very low velocities, it can be converted as a power series to $\gamma = \gamma_0(1 + \beta)$
\4 It is noted that only the radical movement from the observer of the astronomical bodies has an effect on the wavelength, rather than tangential motion
\4 As a result of the formula, $\beta = \frac{\gamma - \gamma_0}{\gamma_0}$, or $v = \frac{|\Delta \gamma|}{\gamma_0}c$
\4 Thus, for bodies moving away, it appears as red shift, whereas for closer, blue shift
\2 For relative velocities not purely radial, the transverse Doppler effect \textbf{FINISH}
\end{outline*}
\end{document}
