...
\1 \textbf{FINISH FROM BOOK WHEN NEEDED (31.7 Onward)}
\end{outline*}
\section{Chapter 32 - Maxwell's Equations and Magnetic Properties}
...
\begin{outline*}
\1 The Earth acts similarly to a large bar magnet, with a magnetic dipole, with the geomagnetic south pole in Antarctica, the geomagnetic north pole NW of Greenland, with the north magnetic pole at the geomagnetic south pole
\2 It is noted that due to cosmic magnetic field interference, the real/dip north pole is located in northern Canada at the Queen Elizabeth Islands instead
\2 The magnetic field at any location is denoted by the field declination (the angle between geographic north and the horizontal component) and the field inclination (the angle between the horizontal plane and field direction)
\3 These are measured ideally with a magnetometer, but can aso be done with a compass and dip meter
\3 The field changes over time at any location substancially as well, though the average field changes extremely slowly over time, reversing polarity every million years
\4 This was found by studying the magnetization of solidified layers of magma mirroring that of Earth over time
\1 Magnetic fields are produced by moving electrons, such as those in a wire, but can also be due to atomic electrons creating a dipole, either by orbitting or spinning
\2 Each atomic electron has an angular momentum, called the spin/spin angular momentum, $\vec{S}$, and a resulting spin magnetic dipole moment, $\vec{\mu}_s$, such that $\vec{\mu}_s = \frac{-e}{m}\vec{S}$
\3 Spin is notable in that it cannot be measured, though its components along an axis can be, and it is quantized, such that each component can only have two values of opposite sign
\4 Spin, while it has angular momentum, does not actually involve the rotation of the electron around some axis, but rather has a quantum mechanic meaning
\3 For some spin measured on the z-axis, $S_z = m_s\frac{h}{2\pi}$, where $m_s$ is the spin magnetic quantum number, for an electron either $\frac{1}{2}$ or $-\frac{1}{2}$, the former called spin up, the latter spin down
\4 As a result, the value of $\mu_s$ on the z-axis can be found, such that it is a constant, called the Bohr magneton, $\mu_B = \frac{eh}{4\pi m} = 9.27 * 10^{-24} J/T$
\3 Thus, for some external magnetic field, there is mangetic energy due to the orientation of the spin magnetic dipole moment, $U = -\frac{\mu_s} \cdot \vec{B}_{ext}$
\3 Protons and neutrons also have a magnetic dipole moment, but it is negligable relative to that of an electron
\2 //ORBITAL MAGNETIC DIPOLE AND BEYOND
\end{outline*}
...
\2 It is shown through these Faraday's Law, using a sinusoidal soltuion, that $\frac{\partial E}{\partial x} = -\frac{\partial B}{\partial t}$, such that E = cB
\3 Further, it is shown similarly by Maxwell's Law that $\mu_0\epsilon_0\frac{\partial E}{\partial x} = -\frac{\partial B}{\partial t}$, following that $E\mu_0\epsilon_0 = B$, such that $c = \frac{1}{\sqrt{\mu_0 \epsilon_0}}$
...
\1 Radiation produces some amount of pressure on an object, due to the linear momentum of the wave, found by Maxwell to be $E = pc$, where E is the energy of the light wave, providing a change in momentum due to a collision with an object
\2 Since $\vec{F} = \frac{d\vec{p}}{dt} = PA$, the momentum change can be related to the pressure to give the radiation pressure
\end{outline*}
\end{document}
